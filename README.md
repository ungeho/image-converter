...
さらに、並び替え、回転・反転、比較スライダー、失敗画像だけの再試行、変換進捗表示、JPEG の EXIF 向き補正、PWA 対応も入っています。
...
4. 必要なら一覧の上下ボタンで順番を並び替えます。
5. 一覧から画像を選び、回転 / 反転や比較スライダーで見た目を確認します。
6. 変換後、個別ダウンロード・一括ダウンロード・ZIP 保存のいずれかで保存します。
7. 失敗画像がある場合は、`失敗画像だけ再試行` で再実行できます。
